...
/** 完成回调 */
@property (nonatomic, copy) void(^completionBlock)(int code, NSArray<ContactModel *> *personArray, NSString *msg);
...
void addressBookChangeCallBack(ABAddressBookRef addressBook, CFDictionaryRef info, void *context)
{
    ...
    // 获取ContactManager单例对象
    ContactManager *contactManager = [ContactManager getInstance];
    if (contactManager.completionBlock) {
        [contactManager getAddressBookPersonListWithSort:contactManager.personSort completionBlock:contactManager.completionBlock];
    }
}
    ...
    switch (ABAddressBookGetAuthorizationStatus()) {
        ...
        case kABAuthorizationStatusRestricted:      //!< 权限被限制.
        {
            if (completionBlock) {
                ...
                completionBlock(-1, nil, @"通讯录访问权限被限制");
            }
        }
            break;
        case kABAuthorizationStatusDenied:          //!< 已拒绝权限.
        {
            if (completionBlock) {
                ...
                completionBlock(-1, nil, @"通讯录访问权限被拒绝");
            }
        }
            break;
...
/**
 ...
 msg 成功或者失败消息
 */
- (void)getAddressBookPersonListWithSort:(PersonSort)personSort completionBlock:(void (^)(int code, NSArray<ContactModel *> *personArray, NSString *msg))completionBlock
{
    // 记录获取参数
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        // 获取通讯录数据
        NSArray<ContactModel *> *personArray = [self getAddressBookPersonListWithSort:personSort];
 ...
 *  同步获取通讯录联系人列表
 *
 *  @return 通讯录ContactModel数组
 */
- (NSArray<ContactModel *> *)getAddressBookPersonListWithSort:(PersonSort)personSort
{
    // 检测权限
    ...
    // 存储通讯录的数组
    NSMutableArray<ContactModel *> *addressBookArray = [NSMutableArray array];
    
    // 循环遍历，获取每个联系人信息
    for (NSInteger i = 0; i < CFArrayGetCount(allPerson); i++)  {
        ABRecordRef person = CFArrayGetValueAtIndex(allPerson, i);
        ContactModel *personModel =  [[ContactModel alloc] initWithPerson:person];
...
- (NSArray<ContactModel *> *)getPersonsWithName:(NSString *)name
{
    ...
    // 存储通讯录的数组
    NSMutableArray<ContactModel *> *personArray = [NSMutableArray array];
...
- (BOOL)addPersons:(NSArray<ContactModel *> *)personArray
{
    ...
    // 遍历数组将联系人添加到通讯录中
    for (ContactModel *personModel in personArray)
    {
        // 实例化CFErrorRef
