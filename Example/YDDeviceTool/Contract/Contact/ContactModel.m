...
- (void)setValueWithPerson:(ABRecordRef)person
{
    ...
    AlternateBirthdayModel *brithdayModel = [[AlternateBirthdayModel alloc] init];
    // 标志
...
//    }
    ...
    /** 电子邮件列表 */
    NSMutableArray<LabelStringModel *> *emailArray = [NSMutableArray array];
    ABMultiValueRef emailRef = ABRecordCopyValue(person, kABPersonEmailProperty);
    for (int i = 0; i < ABMultiValueGetCount(emailRef); i++)
    {
        LabelStringModel *emailModel = [[LabelStringModel alloc] init];
        // 标签
    ...
    /** 地址列表 */
    NSMutableArray<LabelAddressModel *> *addressArray = [NSMutableArray array];
    ABMultiValueRef addressRef = ABRecordCopyValue(person, kABPersonAddressProperty);
    for(int i = 0; i < ABMultiValueGetCount(addressRef); i++)
    {
        LabelAddressModel *addressModel = [[LabelAddressModel alloc] init];
        // 标签
...
//        }
    ...
    /** 电话列表 */
    NSMutableArray<LabelStringModel *> *phoneArray = [NSMutableArray array];
    ABMultiValueRef phoneRef = ABRecordCopyValue(person, kABPersonPhoneProperty);
    for (int i = 0; i < ABMultiValueGetCount(phoneRef); i++)
    {
        LabelStringModel *phoneModel = [[LabelStringModel alloc] init];
        // 标签
    ...
    /** 即时信息列表 */
    NSMutableArray<InstantMessageModel *> *instantMessageArray = [NSMutableArray array];
    ...
    {
        InstantMessageModel *instantMessageModel = [[InstantMessageModel alloc] init];
        // 标签
...
//        }
    ...
    /** URL列表 */
    NSMutableArray<LabelStringModel *> *urlArray = [NSMutableArray array];
    ABMultiValueRef urlRef = ABRecordCopyValue(person, kABPersonURLProperty);
    for (int i = 0; i < ABMultiValueGetCount(urlRef); i++)
    {
        LabelStringModel *urlModel = [[LabelStringModel alloc] init];
        // 标签
    ...
    /** 关联人列表 */
    NSMutableArray<LabelStringModel *> *relatedNameArray = [NSMutableArray array];
    ...
    {
        LabelStringModel *relatedNameModel = [[LabelStringModel alloc] init];
        // 标签
    ...
    /** 社交资料列表 */
    NSMutableArray<SocialProfileModel *> *socialProfileArray = [NSMutableArray array];
    ...
    for (int i = 0 ; i < ABMultiValueGetCount(socialProfileRef); i++)
    {
        ...
        SocialProfileModel *socialProfileModel = [[SocialProfileModel alloc] init];
        // 社交链接
...
- (BOOL)updateToRecordRef:(ABRecordRef)person
{
    ...
    for (LabelStringModel *emailModel in _emails) {
    ...
    for (LabelAddressModel *addressModel in _addresses)
    {
    ...
    for (LabelDateModel *dateModel in _dates) {
    ...
    for (LabelStringModel *phoneModel in _phones) {
    ...
    for (InstantMessageModel *instantMessageModel in _instantMessages)
    {
    ...
    for (LabelStringModel *urlModel in _urls) {
    ...
    for (LabelStringModel *relatedNameModel in _relatedNames) {
    ...
    for (SocialProfileModel *socialProfileModel in _socialProfiles)
    {
    ...
    // 错误消息处理
    if (errorRef) {
        NSLog(@"ContactModel to RecordRef error:%@", errorRef);
        CFRelease(errorRef);
        return NO;
    }
...
/** String标签Model */
@implementation LabelStringModel
...
/** 即时信息Model */
@implementation InstantMessageModel
...
/** 社交资料Model */
@implementation SocialProfileModel
...
/** 农历生日Model */
@implementation AlternateBirthdayModel
...
/** 地址Model */
@implementation LabelAddressModel
